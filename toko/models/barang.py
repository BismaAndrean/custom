...
class barang(models.Model):  # inherit dari Model
    _name = 'toko.barang'  # atribut dari class Model
    ...
    name = fields.Char('Nama Barang', size=64, required=True, index=True, readonly=False)
                       # states={'draft': [('readonly', False)]})
    barang_id = fields.Char('Id Barang', size=64, required=True, index=True, readonly=False)
                          # states={'draft': [('readonly', False)]})
    stock = fields.Integer('Stock', size=64, required=True, index=True, readonly=False)
                           # states={'draft': [('readonly', False)]})
    price = fields.Integer('Price', size=64, required=True, index=True, readonly=False)
                          # states={'draft': [('readonly', False)]})
    date = fields.Date('Date Release', default=fields.Date.context_today, readonly=False)
                       # states={'draft': [('readonly', True)]})
    bara_ids = fields.One2many('toko.detail', 'bara_id', string='Votes')
